...
bool unenrolled(std::string surname){
    ...
    for(int i = 0; i < surnames.size(); i++){
        if(hex(surname) == surnames[i]){                
            ...
            std::ifstream file("D://progects//progect8//task1//text.txt");
            if (file.is_open()) {
                std::string line;
                while (getline(file, line)) {
            ...
            file.close();
            std::ofstream ofs;
            ofs.open("D://progects//progect8//task1//text.txt", std::ofstream::out | std::ofstream::trunc);
    ...
    std::ofstream out("D://progects//progect8//task1//text.txt", std::ios::app);
    if (out.is_open()){
        surname += " 1";
